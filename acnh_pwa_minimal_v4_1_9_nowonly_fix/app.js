...
function getNowDateTime() {
  ...
  // 手動は「月だけ指定」：日付は1日固定、時刻は端末の現在時刻を使用
  const year = now.getFullYear();
  const m = clampInt(s.manualMonth, 1, 12, (now.getMonth()+1));
  const d = 1;
  return new Date(year, m-1, d, now.getHours(), now.getMinutes(), 0, 0);
}
...
function renderList(kind, items){
      ...
      <div class="sectionGrid">
        ...
        <div class="fitem">
          ...
          <div class="row nowrap">
            ...
            <label class="row" style="gap:6px;"><input type="checkbox" id="${kind}-set-anytime" ${s.manualAnytime?"checked":""} ${manualDisabled?"disabled":""}/> <span class="label">すべての時間</span></label>
          </div>
        </div>
  ...
  html += `
      ...
      <div class="small" style="margin-top:10px;">※チェック状態は端末内に保存されます（オフライン可）</div>
    </div>
  `;
...
async function render(){
  ...
  const sel = (active && typeof active.selectionStart==="number" && typeof active.selectionEnd==="number")
